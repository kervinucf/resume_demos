"""
ESPN source — the provider-specific read layer for the sports loader.

Analog of app/lib/sources/geonames.py and open_metro.py: everything that knows
*how ESPN encodes data* lives here and nowhere else. To swap providers, write
another module in app/lib/sources/ exposing the same callables and re-point the
import in app/lib/helpers/sports/__init__.py:

    source_available()                  -> (bool, str)        # transport reachable
    iter_game_candidates(...)           -> Iterator[dict]     # one normalized game per event
...
OPEN_BASE = "https://site.api.espn.com"
...
# ---------------------------------------------------------------------------
# Small primitives (provider-side string/id shaping)
# ---------------------------------------------------------------------------
def slug(value: Any, fallback: str = "unknown") -> str:
    text = _SLUG_RE.sub("-", str(value or "").strip().lower())
    return _DASHES_RE.sub("-", text).strip("-") or fallback


def _key(name: Any, espn_id: Any, fallback: str) -> str:
...
def source_available() -> tuple[bool, str]:
    """Whether the ESPN transport client constructs; and the base url we use."""
    try:
        EspnApiClient()
        return True, OPEN_BASE
    except Exception:  # pragma: no cover - defensive
        return False, OPEN_BASE


# ---------------------------------------------------------------------------
# Event -> neutral record
# ---------------------------------------------------------------------------
def _competitor(c: dict[str, Any]) -> dict[str, Any] | None:
    team = c.get("team") or {}
    ...
    return {
        ...
        "abbreviation": abbr,
        "display_name": name,
        "team_key": _key(name or abbr, team_id, "team"),
...
def _record_from_event(
        event: dict[str, Any], *, league: str, sport: str, season: str, fetched_at: str
) -> dict[str, Any] | None:
    ...
    teams = [t for t in (_competitor(c) for c in competitors) if t is not None]
    if len(teams) < 2:
        return None
    ...
    return {
        ...
        # teams (flat + nested for the factory)
        "home": home,
        "away": away,
        "team_keys": [home["team_key"], away["team_key"]],
        ...
        "venue_key": _key(
            "-".join(x for x in (venue_name, venue_city, venue_country) if x), "", "venue"
        ),
        "fetched_at": fetched_at,
    }


def iter_game_candidates(
        leagues: list[str],
        *,
        date: str | None = None,
    ...
    """
    Stream normalized game records across leagues from ESPN scoreboards.

    ESPN shape knowledge (events[].competitions[0].competitors[], status.type,
    venue.address) lives here only. Leagues that fail to fetch warn and yield
    nothing, mirroring the optional-source behavior in the geo provider.
    """
    espn = EspnApiClient()
    ...
    for league in leagues:
        try:
            ...
            print(f"  warning: scoreboard fetch failed for {league} ({exc})", flush=True)
            continue
        ...
        for event in payload.get("events") or []:
            ...
            record = _record_from_event(
                event, league=league, sport=sport, season=season_guess, fetched_at=fetched_at
            )
